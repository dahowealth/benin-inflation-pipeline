...
print(final_df["Semaine"].isna().sum())
print(final_df["Semaine"].value_counts())
print("*" * 40)
print("-" * 40)
print(
    final_df[
...
final_df[final_df["Semaine"].isna()]["Source_File"].unique()

from sqlalchemy import create_engine

engine = create_engine(
    "mysql+pymysql://root:+229Cadmes@127.0.0.1:3307/Benin_inflation?charset=utf8mb4"
)
...
print("Old SQL data cleared!")

cities_df = final_df[["Ville"]].drop_duplicates()
...
products_df = final_df[["Produit"]].drop_duplicates()
...
print(cities_sql.head())
print(products_sql.head())
...
# 1. Select and rename your columns just like before
...
food_prices_df = food_prices_df.rename(columns={
    "Semaine": "week_label",
    "Prix": "price",
    "Variation": "variation",
...
# 2. NEW: Ensure source_file never exceeds the SQL VARCHAR limit
food_prices_df["source_file"] = food_prices_df["source_file"].astype(str).str[:1024]

# 3. NEW: Convert Pandas NaN values to Python None so MySQL DECIMAL accepts them as NULL
food_prices_df = food_prices_df.where(pd.notnull(food_prices_df), None)

# 4. Upload to SQL safely
food_prices_df.to_sql(
    "food_prices",
    con=engine,
    if_exists="append",
    index=False
